-- Training level 5 first-clear bonus: after the normal 100 gold, grant one
-- random legendary from the five challenge leaders.
...
begin
  ...
  case p_level_id
    ...
    when 'card_types' then v_reward_gold := 100;
    when 'advanced_keywords' then v_reward_gold := 100;
    when 'amp_field' then v_reward_gold := 100;
