-- New accounts should receive the starter pack as owned collection, not the
-- full catalog seed. The starter deck is still created separately so the user
-- can queue immediately, but card_collections must be based on this pack list.
