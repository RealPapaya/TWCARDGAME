-- Cosmetic pack now rolls card-art 炫彩 alongside avatars/titles, with FIXED
-- per-kind odds: 頭像 40% / 稱號 40% / 卡牌炫彩 20%.
--
-- This replaces the previous "uniform pool over all unowned avatar+title" pick
-- (migration 0031/0032) with an explicit per-kind roll. As a consequence, a roll
-- can land on a kind the player has fully collected and pay duplicate
-- compensation — that is the intended trade-off of fixed odds.
--
-- Only the COSMETIC_PACK branch changed vs. the 0032 definition.
...
begin
  ...
  if item.kind in ('CARD_PACK', 'SINGLE_CARD') then
    ...
    for i in 1..coalesce((item.contents->>'itemCount')::integer, 1) loop
      ...
      -- Fixed per-kind odds: 頭像 40% / 稱號 40% / 卡牌炫彩 20%.
      roll := random() * 100;
      chosen_kind := case
        when roll < 40 then 'avatar'
        when roll < 80 then 'title'
        else 'card_art'
      end;
