...
function applyNewsPower(effect: EffectDefinition, context: EffectContext): EffectDefinition {
...
function damageTarget(effect: EffectDefinition, context: EffectContext): void {
...
function damageAndDrawIfKill(effect: EffectDefinition, context: EffectContext): void {
  const ref = getTargetUnit(context.state, context.activeSeat, context.target);
...
function damageSelf(effect: EffectDefinition, context: EffectContext): void {
...
function damageNonCategory(effect: EffectDefinition, context: EffectContext): void {
  const ref = getTargetUnit(context.state, context.activeSeat, context.target);
  if (!ref || ref.kind !== "MINION") return;
  const minion = ref.unit as RuntimeMinion;
...
function damageAllNonCategories(effect: EffectDefinition, context: EffectContext): void {
...
function damageAllEnemyMinions(effect: EffectDefinition, context: EffectContext): void {
...
function damageRandomFriendly(effect: EffectDefinition, context: EffectContext): void {
...
function healTarget(effect: EffectDefinition, context: EffectContext): void {
  const ref = getTargetUnit(context.state, context.activeSeat, context.target);
  if (ref) healUnit(context.state, ref, effect.value ?? 0, context.events);
}

function healCategoryBonus(effect: EffectDefinition, context: EffectContext): void {
  const ref = getTargetUnit(context.state, context.activeSeat, context.target);
  if (!ref) return;
...
function fullHeal(effect: EffectDefinition, context: EffectContext): void {
  const ref = getTargetUnit(context.state, context.activeSeat, context.target);
  if (!ref) return;
...
function healAllFriendly(_effect: EffectDefinition, context: EffectContext): void {
  const player = context.state.players[context.activeSeat];
...
function fullHealAndDraw(effect: EffectDefinition, context: EffectContext): void {
  fullHeal(effect, context);
  drawCards(context.state, context.state.players[context.activeSeat], 1, context.events);
}

function buffStatTarget(effect: EffectDefinition, context: EffectContext): void {
  const ref = getTargetUnit(context.state, context.activeSeat, context.target);
  if (!ref || ref.kind !== "MINION") return;
  buffMinion(context.state, ref.owner, ref.unit as RuntimeMinion, effect.stat ?? "ATTACK", effect.value ?? 0, context.events);
}

function buffStatTargetCategoryBonus(effect: EffectDefinition, context: EffectContext): void {
  const ref = getTargetUnit(context.state, context.activeSeat, context.target);
  if (!ref || ref.kind !== "MINION") return;
  const minion = ref.unit as RuntimeMinion;
  const value = minion.category.includes(effect.target_category_includes ?? "") ? effect.bonus_value ?? effect.value ?? 0 : effect.value ?? 0;
...
function buffStatTargetTemp(effect: EffectDefinition, context: EffectContext): void {
  const ref = getTargetUnit(context.state, context.activeSeat, context.target);
  if (!ref || ref.kind !== "MINION") return;
  const minion = ref.unit as RuntimeMinion;
...
function buffHealthAndTauntTarget(effect: EffectDefinition, context: EffectContext): void {
  const ref = getTargetUnit(context.state, context.activeSeat, context.target);
  if (!ref || ref.kind !== "MINION") return;
  const minion = ref.unit as RuntimeMinion;
...
function buffAll(effect: EffectDefinition, context: EffectContext): void {
  const player = context.state.players[context.activeSeat];
  for (const minion of player.board) buffMinion(context.state, player, minion, effect.stat ?? "ALL", effect.value ?? 0, context.events);
}

function buffCategory(effect: EffectDefinition, context: EffectContext): void {
  const player = context.state.players[context.activeSeat];
  for (const minion of player.board) {
...
function buffAdjacent(effect: EffectDefinition, context: EffectContext): void {
...
function giveDivineShield(effect: EffectDefinition, context: EffectContext): void {
  const ref = getTargetUnit(context.state, context.activeSeat, context.target);
  if (!ref || ref.kind !== "MINION") return;
...
function giveDivineShieldAll(_effect: EffectDefinition, context: EffectContext): void {
  const player = context.state.players[context.activeSeat];
  for (const minion of player.board) {
...
function giveDivineShieldCategory(effect: EffectDefinition, context: EffectContext): void {
  const player = context.state.players[context.activeSeat];
  for (const minion of player.board) {
...
function giveKeywordAdjacent(effect: EffectDefinition, context: EffectContext): void {
  const source = sourceMinionInPlay(context);
...
function lockAttack(effect: EffectDefinition, context: EffectContext): void {
  const ref = getTargetUnit(context.state, context.activeSeat, context.target);
  if (!ref || ref.kind !== "MINION") return;
  const minion = ref.unit as RuntimeMinion;
...
function lockSelf(effect: EffectDefinition, context: EffectContext): void {
  const source = sourceMinionInPlay(context);
  if (!source) return;
...
function lockAllEnemy(effect: EffectDefinition, context: EffectContext): void {
  const enemy = context.state.players[opponentOf(context.activeSeat)];
  for (const minion of enemy.board) {
    minion.lockedTurns = Math.max(minion.lockedTurns, effect.value ?? 1);
    minion.canAttack = false;
...
function lockAllAndBuffCategory(effect: EffectDefinition, context: EffectContext): void {
  forEachMinion(context.state, (player, minion) => {
    minion.lockedTurns = Math.max(minion.lockedTurns, effect.value ?? 1);
    minion.canAttack = false;
    if (minion.category === effect.target_category) {
...
function unlockAndBuffHealth(effect: EffectDefinition, context: EffectContext): void {
  const ref = getTargetUnit(context.state, context.activeSeat, context.target);
  if (!ref || ref.kind !== "MINION") return;
  const minion = ref.unit as RuntimeMinion;
...
function summonMultiple(effect: EffectDefinition, context: EffectContext): void {
...
function addCardToHand(effect: EffectDefinition, context: EffectContext): void {
  const card = effect.cardId ? context.catalog.get(effect.cardId) : undefined;
  if (!card) return;
  const player = context.state.players[context.activeSeat];
...
function destroyTarget(effect: EffectDefinition, context: EffectContext): void {
  const ref = getTargetUnit(context.state, context.activeSeat, context.target);
  if (ref?.kind === "MINION") (ref.unit as RuntimeMinion).currentHealth = 0;
}

function destroyAllMinions(_effect: EffectDefinition, context: EffectContext): void {
  forEachMinion(context.state, (_player, minion) => {
    minion.currentHealth = 0;
  });
}

function destroyLowAttack(effect: EffectDefinition, context: EffectContext): void {
  destroyByAttack(context, (attack) => attack <= (effect.value ?? 0));
}

function destroyHighAttack(effect: EffectDefinition, context: EffectContext): void {
  destroyByAttack(context, (attack) => attack >= (effect.value ?? 0));
}

function destroyDamaged(_effect: EffectDefinition, context: EffectContext): void {
  const ref = getTargetUnit(context.state, context.activeSeat, context.target);
  if (ref?.kind === "MINION") {
    const minion = ref.unit as RuntimeMinion;
...
function destroyLocked(_effect: EffectDefinition, context: EffectContext): void {
  const ref = getTargetUnit(context.state, context.activeSeat, context.target);
...
function discardDraw(effect: EffectDefinition, context: EffectContext): void {
...
function discardRandom(effect: EffectDefinition, context: EffectContext): void {
  const player = context.state.players[context.activeSeat];
...
function drawEffect(effect: EffectDefinition, context: EffectContext): void {
  drawCards(context.state, context.state.players[context.activeSeat], effect.value ?? 1, context.events);
}

function drawMinionReduceCost(effect: EffectDefinition, context: EffectContext): void {
  const player = context.state.players[context.activeSeat];
...
function drawNews(effect: EffectDefinition, context: EffectContext): void {
  const player = context.state.players[context.activeSeat];
  const index = player.deck.findIndex((card) => card.type === "NEWS");
  if (index !== -1) drawCards(context.state, player, 1, context.events, index, effect.value ?? 0);
}

function eatFriendly(effect: EffectDefinition, context: EffectContext): void {
  const source = sourceMinionInPlay(context);
  const ref = getTargetUnit(context.state, context.activeSeat, context.target);
...
function multiDamage(effect: EffectDefinition, context: EffectContext): void {
  const enemy = context.state.players[opponentOf(context.activeSeat)];
...
function reduceCostAllHand(effect: EffectDefinition, context: EffectContext): void {
  const player = context.state.players[context.activeSeat];
...
function setAttackAll(effect: EffectDefinition, context: EffectContext): void {
  forEachMinion(context.state, (player, minion) => {
...
function setDeathTimer(effect: EffectDefinition, context: EffectContext): void {
  const ref = getTargetUnit(context.state, context.activeSeat, context.target);
  if (ref?.kind === "MINION") (ref.unit as RuntimeMinion).deathTimer = effect.value ?? 3;
}

function swapAttackHealth(_effect: EffectDefinition, context: EffectContext): void {
  const ref = getTargetUnit(context.state, context.activeSeat, context.target);
  if (!ref || ref.kind !== "MINION") return;
  const minion = ref.unit as RuntimeMinion;
...
function bounceTarget(_effect: EffectDefinition, context: EffectContext): void {
  const ref = getTargetUnit(context.state, context.activeSeat, context.target);
  if (ref?.kind === "MINION") bounceMinion(context.state, ref.owner, ref.unit as RuntimeMinion, context.catalog, context.events);
}

function bounceCategory(effect: EffectDefinition, context: EffectContext): void {
  const ref = getTargetUnit(context.state, context.activeSeat, context.target);
...
function bounceAllCategory(effect: EffectDefinition, context: EffectContext): void {
...
function bounceAllEnemy(effect: EffectDefinition, context: EffectContext): void {
  const enemy = context.state.players[opponentOf(context.activeSeat)];
...
function bounceRandomEnemy(_effect: EffectDefinition, context: EffectContext): void {
  const enemy = context.state.players[opponentOf(context.activeSeat)];
