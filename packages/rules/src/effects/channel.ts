...
/** 通靈 / Discover: how many candidate cards are revealed when `count` is omitted. */
const DEFAULT_REVEAL = 3;
/** Picker label shown in the UI / events. */
const CHANNEL_LABEL = "通靈";

/**
 * 通靈 (CHANNEL / Discover). Pulls up to `count` (default 3) random cards out of the
...
/**
 * Resolves an open 通靈 choice: the picked card joins the seat's hand (burned if the
