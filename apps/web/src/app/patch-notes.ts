...
export const PATCH_NOTES: PatchNoteVersion[] = [
  {
    ...
    items: [
      ...
      { title: "[新增增幅] 複數好運一番賞效果與重抽機制", desc: "新增「壽險理賠」、「定期定額」、「台股四萬點」、「跳樓大拍賣」、「一黨獨大」、「台雞電OFFER」、「林北檳榔擠剛攏哺500啦」、「里長的愛心便當」、「捐血送禮券」、「流水席」、「國定假日」、「黨產大補丸」等多張好運一番賞，並重製「蹲得越低」效果，且實作了增幅重抽（Reroll）與相關 UI。" },
